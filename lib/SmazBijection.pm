...
sub flush_verbatim {
	...
	return (((length($verbatim) > 1)
		? ( bin(255), bin(length($verbatim) - 1) )
		: bin(254)), map { bin(ord($_)) }split '', $verbatim);
}

sub bin {
	return substr(Math::BigInt->new(shift)->as_bin(), 2);
}
